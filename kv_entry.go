...
// Entry represents a single mutation record in the Write-Ahead Log (WAL).
// It captures the key, value, and whether the operation was a deletion.
type Entry struct {
	key     []byte
	val     []byte
	deleted bool
...
// Binary layout constants for the Entry header.
const (
	lengthSize      = 8
	entryHeaderSize = 4 * lengthSize
)

// ErrBadSum is returned when the decoded data's checksum does not match 
// the stored checksum, indicating data corruption.
var ErrBadSum = errors.New("bad checksum")

// tab is the ISO-compliant CRC64 table used for calculating data integrity hashes.
var tab *crc64.Table = crc64.MakeTable(crc64.ISO)

// Encode serializes the Entry into a binary format suitable for persistent storage.
// It uses Little Endian encoding for integers and includes a CRC64 checksum 
// to ensure data integrity during recovery.
//
// Binary Format:
// | key size (8B) | val size (8B) | checksum (8B) | deleted (8B) | key data | val data |
...
// Decode reads and deserializes an Entry from the provided reader. 
// It validates the payload against the stored CRC64 checksum and 
// returns ErrBadSum if a mismatch is detected.
